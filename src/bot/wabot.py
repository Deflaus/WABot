...
class WABot:
    ...
    def send_message_to_user(self, chatID):
        ...
        try:
            ...
            return self.send_message(
                chatID, f"Сотруднику с номером телефона {info[1]} отправлено сообщение"
            )
        except IndexError:
            return self.send_message(
                chatID,
    ...
    def send_message_to_department(self, chatID):
        ...
        try:
            ...
            for user in users:
                self.send_message(
                    user.phone_number, f"Сообщение от {chatID}:\n{info[2]}"
                )
            return (
                self.send_message(
                    chatID, f"Сотрудникам отдела {info[1]} отправлены сообщения"
                )
                if users
                else self.send_message(chatID, f"Такого отдела не существует")
            )
        except IndexError:
            return self.send_message(
                chatID,
